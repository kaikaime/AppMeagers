...
import com.example.lvkaixue.appmeager.single.ActivityCollector;
...
long times;
    protected  void checkUserIsLogin(){
        //判断用户是否登录如果没有登录就让用户进入登录界面
      if ((times = (System.currentTimeMillis() - sp.getLong("outTime",0))) > 300000000) {
                    //检测是否登录过期
                    SingleUser.getSingleUser().setAccessToken("");
                    //如果是在登录页面，就不需要在此跳转到LoginActivity
                    if(this.getClass().getSimpleName().equals(LoginActivity.activityName)){return;}
