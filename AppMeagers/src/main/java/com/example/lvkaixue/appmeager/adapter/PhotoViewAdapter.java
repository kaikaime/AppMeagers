...
public class PhotoViewAdapter {
    ...
    public static class PhotoReclyViewAdapter extends RecyclerView.Adapter<MyViewHodler>{
        ...
        public PhotoReclyViewAdapter(Context context,PhotoBean photoBean) {
            ...
            for(int i = 0;i<photoBean.album.size();i++){
                System.out.println("相册名称: "+photoBean.album.get(i).name+"  "+photoBean.album.get(i).albumid+"  "+photoBean.album.get(i).coverurl+"   "+photoBean.album.get(i).classid);
            }
        }
